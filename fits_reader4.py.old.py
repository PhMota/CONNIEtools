...
def plotrunID( ohdu, *FITSfiles, **kwargs ):
    ...
    if plot:
        ...
        fig = plt.figure()
        fig.set_size_inches( np.array(fig.get_size_inches())*[2,1] )
        fig.suptitle('runID%s'%runID+' ohdu%s'%ohdu)
        first = True
        if fft:
            gs = GridSpec(5,2)
            ax = [ fig.add_subplot(gs[:-2,i], sharex = None if first else ax[0], sharey = None if first else ax[0] ) for i in [0,1] ]
            axdiff = [ fig.add_subplot(gs[-2,i], sharex=ax[i], sharey = None if first else axdiff[0] ) for i in [0,1] ]
            axfft = [ fig.add_subplot(gs[-1,i]) for i in [0,1] ]
            first = False
        for ax_ in ax: 
    ...
    for FITSfile in FITSfiles:
        ...
        el, elchi2 = zip( *[ fit( xbins, histOS, logGaussx, sel=[ histOS>countCut ], log=True ) for histOS in histOSlist ] )
        ...
        elx, elxchi2 = zip( *[ fit( xbins, histOSx, logGauss, sel=[ histOSx>countCut ], log=True ) for histOSx in histOSxlist ] )
        ...
        print 'fitting log(histAC>%s,xbins>0) with quadratic function'%countCut
        elxpos, elxposchi2 = zip( *[fit( xbins, histAC, logGauss, sel=[ xbins>0, histAC>countCut ], log=True ) for histAC in histACxlist ] )
        if first: 
        ...
        if plot:
            ...
            for i in [0,1]:
                ...
                ax[i].step( xbins, histOSlist[i], where='mid', label = 'os' )
                ax[i].step( xbins, histOSxlist[i], where='mid', label = 'osx' )
                ax[i].step( xbins, histAClist[i], where='mid', label = 'ac' )
                ax[i].step( xbins, histACxlist[i], where='mid', label = 'acx' )
                gOS = gaussian(xbins,*elx[i])
                axdiff[i].step( xbins[gOS>countCut], np.abs(histOSlist[i][gOS>countCut] - gOS[gOS>countCut])/gOS[gOS>countCut], where='mid', label = 'os(%.2f)'%(elx[i][0]), color = 'C1' )
                gACneg = gaussian(xbins,*elxneg[i])
                axdiff[i].step( xbins[gACneg>countCut], np.abs(histAClist[i][gACneg>countCut] - gACneg[gACneg>countCut])/gACneg[gACneg>countCut], where='mid', label = 'ac<(%.2f)'%(elxneg[i][0]), color = 'C2' )
                gACpos = gaussian(xbins,*elxpos[i])
                axdiff[i].step( xbins[gACpos>countCut], np.abs(histAClist[i][gACpos>countCut] - gACpos[gACpos>countCut])/gACpos[gACpos>countCut], where='mid', label = 'ac>(%.2f)'%(elxpos[i][0]), color = 'C3' )
        ...
        if fft:
            ...
            for i in [0,1]:
                ...
                fftAC = np.fft.fft( histAClist[i][maskAC] )
                axfft[i].step( xbins[maskAC], np.fft.fftshift( np.abs(np.real( fftAC )) ), where='mid', label = 'fftac' )
                axfft[i].step( xbins[maskAC], np.fft.fftshift( np.abs(np.imag( fftAC )) ), where='mid', label = 'fftac' )
                maskOS = histOSlist[i]>countCut
                fftOS = np.fft.fft( histOSlist[i][maskOS] ) 
                axfft[i].step( xbins[maskOS], np.fft.fftshift( np.abs(np.real(fftOS)) ), where='mid', label = 'fftos' )
                axfft[i].step( xbins[maskOS], np.fft.fftshift( np.abs(np.imag(fftOS)) ), where='mid', label = 'fftos' )
    ...
    if plot:
        ...
        print 'plot generated in PNG file', '%s.png'%(FITSfile)
        #fig.savefig('ccd%s_%s.png'%(runID,ohdu))
        fig.savefig('%s.png'%(FITSfile))
    
    print 'table generated in CSV file', '%s.csv'%(FITSfile)
    np.savetxt('%s.csv'%(FITSfile), result, header=', '.join(header), fmt=' '.join(fmt), delimiter=', ')
